...
| Voice snippets                                               | Added: local plain text and line breaks, whole-cue and in-sentence matching, longest phrase first, preview, save/edit/remove/undo; 1,000 entries. No rich-text snippets or team sharing. |
...
| App-aware writing styles                                     | Added basic local casing/punctuation rules by desktop app name. No browser URL categories or AI tone matching.                                                                           |
...
Custom accents, a floating macOS dock, explicit app/link voice actions, and a preview-based Wispr library importer are implemented. Optional history imports retain text and timestamps only. [Setup and limits](personalization.md). These additions do not establish full Flow parity.
